...
Our first intention was to implement multi-tenancy associating for each tenant a different DBSchema placed in a single DB: we wanted to access from a single DB-table, containing <User,Password,Tenant>, differents DBSchema's in the same DB.  
